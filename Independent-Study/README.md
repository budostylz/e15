...
### Let's turn to our `package.json`, add/set `private` attribute to `true`, finally removing our `main` entry since we'll define that in our `webpack.config.js` in the next steps.
